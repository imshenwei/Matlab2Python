...
def read_wav(wav_path):
    ...
    return mic_signal, framerate, nframes


mic_signal, framerate, nframes = read_wav(wav_path)


def getMicsignal(RESPEAKER_RATE=16000, CHUNK=1024, RECORD_SECONDS=0.02):
    mics = mic.mic_array()

    data = mics.get_data(RESPEAKER_RATE / CHUNK * RECORD_SECONDS)
    framerate = RESPEAKER_RATE
    mic_signal = data
    nframes = RECORD_SECONDS*RECORD_SECONDS
    return mic_signal, framerate, nframes


mic_signal, framerate, nframes = getMicsignal()
...
# mic_signal = simulateArraydata(source_info, mic_pos, c, framerate, source_duration, mic_centre);
