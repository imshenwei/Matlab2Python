...
def simulateArraydata(source_info, mic_info, c, fs, duration, mic_centre):
    ...
    t = np.arange(0, (duration[0]-1/fs)+1/fs, 1/fs).reshape(1, -1)
    ...
    for I_I in range(1, N_source+1):
        # 阵列中心到声源的距离
        r_source_to_centre = norm(mic_centre-source_info[I_I-1, 0:3])
        # 信号功率为 amp^2
        # 声压级 SPL = 20*log10(amp/2e-5), 我们通过从源到阵列中心的距离进行缩放以获得中心处的给定 SPL
        amp = 2e-5*10**(source_info[I_I-1, 4]/20)
        for J in range(1, N_mic+1):
            # 第 J 个麦克风到仿真声源点的距离
            r_source_to_mic = math.sqrt(np.dot(
